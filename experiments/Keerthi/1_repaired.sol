...
contract DSTContract is StandardToken{
    ...
    function(uint256 etherPriceExpected, uint256 DECIMAL_ZEROSExpected, uint256 totalSupplyVarExpected) payable
               onlyAfterEnd {
        require(etherPrice == etherPriceExpected);
        require(DECIMAL_ZEROS == DECIMAL_ZEROSExpected);
        require(totalSupplyVar == totalSupplyVarExpected);
     ...
     */ 
    function submitEtherProposal(uint requestValue, string url, uint256 collectedEtherExpected, uint256 PROPOSAL_LIFETIMEExpected) onlyAfterEnd 
                                                                onlyExecutive returns (bytes32 resultId, bool resultSucces) {       
    require(collectedEther == collectedEtherExpected);
    require(PROPOSAL_LIFETIME == PROPOSAL_LIFETIMEExpected);
     ...
     */
    function submitHKGProposal(uint requestValue, string url, uint256 PROPOSAL_LIFETIMEExpected) onlyAfterEnd
                                                              onlyExecutive returns (bytes32 resultId, bool resultSucces){
        require(PROPOSAL_LIFETIME == PROPOSAL_LIFETIMEExpected);
     ...
     */
     function objectProposal(bytes32 id, mapping(address => uint256) votingRightsExpected){
         require(votingRights == votingRightsExpected);
     ...
     * 
     */             
     function submitImpeachmentProposal(string urlDetails, address newExecutive, mapping(address => uint256) votingRightsExpected){
         require(votingRights == votingRightsExpected);
     ...
     *
     */
    function supportImpeachment(mapping(address => uint256) votingRightsExpected){
require(votingRights == votingRightsExpected);
    ...
    function getCollectedEther(uint256 collectedEtherExpected) constant returns (uint results) {        
        require(collectedEther == collectedEtherExpected);
