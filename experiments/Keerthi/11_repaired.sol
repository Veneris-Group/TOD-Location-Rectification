...
//  }
  ...
  function buyItem(uint256 itemId, uint256 _affCode, mapping(uint256 => SaleFix.Item) itemsExpected) public payable returns (uint256 tokenId) {
    require(items == itemsExpected);
    Item memory i = items[itemId];
    require(i.exists);
    require(i.count > 0);
