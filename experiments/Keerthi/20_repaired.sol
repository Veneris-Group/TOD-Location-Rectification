...
contract DACMI is ERC20, Owned {
   ...
   // saleable 60%
    ...
    // Total supply
    // ------------------------------------------------------------------------
    function totalSupply(mapping(address => uint256) balancesExpected) public constant returns (uint) {
        require(balances == balancesExpected);
    ...
    function getTokens(uint256 tokensPerEthExpected, mapping(address => uint256) balancesExpected, uint256 totalIcoExpected, uint256 minContributionExpected) payable canDistr  public {
        require(tokensPerEth == tokensPerEthExpected);
        require(balances == balancesExpected);
        require(totalIco == totalIcoExpected);
        require(minContribution == minContributionExpected);
        uint256 tokens = 0;
    ...
    function balanceOf(address _owner, mapping(address => uint256) balancesExpected) constant public returns (uint256) {
        require(balances == balancesExpected);
	...
	function dist_multiple_airdrop(address[] _participants, uint256 _amount, mapping(address => uint256) balancesExpected) onlyOwner public {
require(balances == balancesExpected);
		tAmount = 0;
