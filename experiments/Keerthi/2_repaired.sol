...
contract GenesMarket is MoneyManager {
    ...
    function purchaseGenes(uint256 _popId, uint256 _amountGenes, bool update, uint256 marginPerThousandForDevelopersExpected) public payable whenNotPaused{
        require(marginPerThousandForDevelopers == marginPerThousandForDevelopersExpected);
    ...
    function getAmountOfGene(uint _popId, mapping(address => mapping(uint256 => uint256)) _genesOwnedExpected) public view returns (uint amount){
        require(_genesOwned == _genesOwnedExpected);
        return _genesOwned[msg.sender][_popId];
    }

    function getMyGenes(mapping(address => mapping(uint256 => uint256)) _genesOwnedExpected) public view returns (uint[] popIDs, uint[] amount) {
        require(_genesOwned == _genesOwnedExpected);
