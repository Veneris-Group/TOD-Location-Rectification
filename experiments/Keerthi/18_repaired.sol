...
contract modERC20 is IERC20 {
    ...
    function getHolders(address[] holdersExpected) public view returns (address[]) {
        require(holders == holdersExpected);
        return holders;
    }
...
contract InvestToken is modERC20, ERC20Detailed, Ownable {
    ...
    function buyTokens(address referrer, uint256 MIN_HOLDERS_BALANCEExpected) public payable {
        require(MIN_HOLDERS_BALANCE == MIN_HOLDERS_BALANCEExpected);
    ...
    function swapTokens(uint256 tokensAmountToBurn, uint8 TOKENS_DIVIDERExpected) public {
        require(TOKENS_DIVIDER == TOKENS_DIVIDERExpected);
    ...
    function sendDividendsToHolders(uint holdersIterations, address[] holdersExpected) public onlyOwner {
        require(holders == holdersExpected);
        if (holdersIndex == 0) {
