...
contract MultiPadLaunchApp is IBEP20{
    ...
    */
     ...
     */
     function buyTokens(uint256 _contribution, IBUSD iBUSDExpected) external checkSaleValidations(msg.sender, _contribution) returns(bool){
         require(iBUSD == iBUSDExpected);
     ...
     */
    function claimPurchasedTokens(uint256 _lockNumber, IMainToken iMainTokenExpected) external validateClaim(msg.sender,_lockNumber) returns (bool){
        require(iMainToken == iMainTokenExpected);
    ...
    function withdrawPeningTokens(uint256 _amount, IBUSD iBUSDExpected) external onlyOwner returns(bool){
       require(iBUSD == iBUSDExpected);
