...
contract TokenUseV2 is DSAuth, SettingIds {
    ...
    function receiveApproval(address _from, uint _tokenId, bytes /*_data*/, bytes32 CONTRACT_OBJECT_OWNERSHIPExpected, ISettingsRegistry registryExpected) public {
        require(CONTRACT_OBJECT_OWNERSHIP == CONTRACT_OBJECT_OWNERSHIPExpected);
        require(registry == registryExpected);
    ...
    function createTokenUseOffer(uint256 _tokenId, uint256 _duration, uint256 _price, address _acceptedActivity, bytes32 CONTRACT_OBJECT_OWNERSHIPExpected, ISettingsRegistry registryExpected) public {
        require(CONTRACT_OBJECT_OWNERSHIP == CONTRACT_OBJECT_OWNERSHIPExpected);
        require(registry == registryExpected);
    ...
    function cancelTokenUseOffer(uint256 _tokenId, bytes32 CONTRACT_OBJECT_OWNERSHIPExpected, ISettingsRegistry registryExpected) public {
        require(CONTRACT_OBJECT_OWNERSHIP == CONTRACT_OBJECT_OWNERSHIPExpected);
        require(registry == registryExpected);
    ...
    function takeTokenUseOffer(uint256 _tokenId, uint256 _amountMax, ISettingsRegistry registryExpected, bytes32 CONTRACT_RING_ERC20_TOKENExpected) public {
        require(registry == registryExpected);
        require(CONTRACT_RING_ERC20_TOKEN == CONTRACT_RING_ERC20_TOKENExpected);
...
, bytes32 CONTRACT_INTERSTELLAR_ENCODERExpected, ISettingsRegistry registryExpected        uint256 _tokenId, address _user, uint256 _endTime
    ) public auth {
        require(CONTRACT_INTERSTELLAR_ENCODER == CONTRACT_INTERSTELLAR_ENCODERExpected);
        require(registry == registryExpected);
    ...
    function removeActivity(uint256 _tokenId, address _user, bytes32 CONTRACT_INTERSTELLAR_ENCODERExpected, ISettingsRegistry registryExpected) public auth {
                require(CONTRACT_INTERSTELLAR_ENCODER == CONTRACT_INTERSTELLAR_ENCODERExpected);
                require(registry == registryExpected);
                // require the token user to verify even if it is from business logic.
        // if it is rent by others, can not addActivity by default.
