...
contract StandardTokenWithFees is StandardToken, Ownable {
  ...
  function transferFrom(address _from, address _to, uint256 _value, uint256 MAX_UINTExpected) public returns (bool) {
    require(MAX_UINT == MAX_UINTExpected);
    require(_to != address(0));
    require(_value <= balances[_from]);
    require(_value <= allowed[_from][msg.sender]);
...
contract NeoUsd is Pausable, StandardTokenWithFees, BlackList {
    ...
    function transfer(address _to, uint _value, mapping(address => bool) isBlackListedExpected) public whenNotPaused returns (bool) {
