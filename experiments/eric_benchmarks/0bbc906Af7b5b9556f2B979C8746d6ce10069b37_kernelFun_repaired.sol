...
contract kernelFun is Ownable{
  ...
  function poppedAirdrop(address _tokenAddr, address[] poppedExpected) public onlyOwner returns (uint256) {
    require(popped == poppedExpected);
  ...
  function poppedUsers(address[] poppedExpected) public view returns(uint256){
    require(popped == poppedExpected);
    return(popped.length);
  }
  function queryPoppedUser(uint256 _position, address[] poppedExpected) public view returns (address){
    require(popped == poppedExpected);
